...
class HealthCheckRegistryTest {
    ...
    void test(Vertx vertx, VertxTestContext testContext) throws NoSuchFieldException, IllegalAccessException {
        ...
        WriteSafeRegistry registry = new WriteSafeRegistry(vertx, HealthCheckRegistry.REGISTRY_NAME);
        Future<Void> setupFuture = registry.getSharedMap()
                .compose(sharedMap -> sharedMap.put(SHARED_MAP_KEY, new JsonArray().add(verticleName)));
        ...
        setupFuture
                ...
                .compose(result -> registry.getSharedMap()).compose(map -> map.get(SHARED_MAP_KEY))
                .map(JsonArray.class::cast).onSuccess(registeredVerticles -> testContext.verify(() -> {
