...
class EntityVerticleTest extends EntityVerticleTestBase {
    ...
    void registerEntityTypes(VertxTestContext testContext) {
        WriteSafeRegistry registry = new WriteSafeRegistry(getNeonBee().getVertx(), EntityVerticle.REGISTRY_NAME);

        Checkpoint checkpoint = testContext.checkpoint(2);
        registry.getSharedMap().compose(map -> map.get(sharedEntityMapName(new FullQualifiedName("ERP.Customers"))))
        ...
        registry.getSharedMap().compose(map -> map.get(sharedEntityMapName(new FullQualifiedName("Sales.Orders"))))
                .onComplete(testContext.succeeding(result -> {
                    testContext.verify(() -> {
                        assertThat((JsonArray) result).containsExactly(entityVerticleImpl1.getQualifiedName());
                    });
